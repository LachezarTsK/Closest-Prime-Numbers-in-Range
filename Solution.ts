...
const SIEVE_FOR_PRIMES: boolean[] = createSieveOfEratosthenes();

function closestPrimes(lowerLimit: number, upperLimit: number): number[] {
    const closestPrimes: number[] = [RANGE_OF_VALUES[0], RANGE_OF_VALUES[1]];
...
function findFirstPrimeNumberInRange(lowerLimit: number, upperLimit: number): number {
...
function updateClosestPrimes(closestPrimes: number[], previousPrime: number, currentPrime: number): void {
...
function createSieveOfEratosthenes(): boolean[] {
    const sieveForPrimes: boolean[] = new Array(RANGE_OF_VALUES[1] + 1).fill(true);
